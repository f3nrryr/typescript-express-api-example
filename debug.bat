...
for /L %%i in (1,1,30) do (
  docker exec debugDbPg pg_isready -U postgres >nul 2>&1
  if !errorlevel! equ 0 (
...
docker exec debugDbPg psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='pastgregre'" | findstr /C:"1" >nul
if !errorlevel! equ 0 (
  echo База данных "pastgregre" уже существует.
) else (
  echo Создаём базу данных "pastgregre"...
  docker exec debugDbPg psql -U postgres -c "CREATE DATABASE \"pastgregre\""
  if !errorlevel! equ 0 (
